...
const game: SearchForFunGame = {
  id: "n_0011_pogo_storm_surge_battery",
  title: "Pogo Storm: Surge Battery",
  instructions: "Left / Right steer. Space spends STOMP charge in the air. R restarts.",

  mount(k: KAPLAYCtx, playtest, runtime) {
    const W = 640;
    const H = 640;
    const runLength = 84;
    const stompEnergyMax = 100;
    const stompEnergyCost = 44;
    const stompEnergyRegen = 24;
    const stompEnergyDelay = 0.45;
    const hordeHardCap = 125;
    ...
    let stompAge = 0;
    let stompCooldown = 0;
    let stompEnergy = stompEnergyMax;
    let stompEnergyDelayClock = 0;
    let blockedStompTelemetryClock = 0;
    let stompSpends = 0;
    let blockedStomps = 0;
    let invulnerable = 0;
    let health = 5;
    ...
    let refillBudget = 0;
    let nextSurgeAt = 6.4;
    let surgeTelegraphClock = 0;
    let surgeQueued = false;
    let surgeIndex = 0;
    let surgesReleased = 0;
    let hordePeak = 1;
    let nextRampCheckpoint = 0;
    let lancerClock = 0;
    let phaseCharges = 0;
    let phaseRechargeClock = 0;
    const unlocked = new Set<PowerKind>();
    const triggered = new Set<PowerKind>();
    const rampCheckpoints = [0, 10, 20, 30];
    ...
    const titleText = k.add([k.text("POGO STORM  •  SURGE BATTERY", { size: 20 }), k.pos(24, 18), k.color(pale), k.z(50)]);
    ...
    const energyText = k.add([k.text("STOMP 100  CHARGED", { size: 12 }), k.pos(25, 82), k.color(accent), k.z(50)]);
    const hordeText = k.add([k.text("HORDE 001/001", { size: 12 }), k.pos(615, 82), k.anchor("topright"), k.color(accent), k.z(50)]);
    k.add([k.rect(160, 9), k.pos(166, 82), k.color(dark), k.outline(1, mid), k.z(49)]);
    const energyFill = k.add([
      k.rect(156, 5), k.pos(168, 86), k.anchor("left"), k.color(accent), k.opacity(0.92), k.scale(1, 1), k.z(50),
    ]);
    k.add([
      k.rect(1, 11), k.pos(168 + 156 * (stompEnergyCost / stompEnergyMax), 81), k.color(pale), k.opacity(0.72), k.z(51),
    ]);
    const powerText = k.add([k.text("◇ CAP   ◇ LANCE   ◇ PHASE", { size: 12 }), k.pos(398, 101), k.color(mid), k.z(50)]);
    const patternText = k.add([k.text("GROUND SKITTERS", { size: 12 }), k.pos(25, 101), k.color(accent), k.z(50)]);
    ...
    const burst = (x: number, y: number, amount: number, bright = true) => {
      const available = Math.max(0, 240 - sparks.length);
    ...
    const begin = (method: string) => {
      ...
      playtest.event("horde_ramp", { checkpoint: 0, active: foes.length, target: 1, second: 0 });
      nextRampCheckpoint = 1;
    ...
    const spawnFoe = (kind: ThreatKind, side?: "left" | "right", lane = 0): boolean => {
      if (foes.length >= hordeHardCap) return false;
    ...
    const targetHordeAt = (seconds: number) => 1 + Math.floor(clamp(seconds / 30, 0, 1) * 99);

    const refillKind = (): ThreatKind => {
      if (elapsed < 4) return "skitter";
      const roll = random();
      if (currentPattern === "hover") return roll < 0.44 ? "hover" : "skitter";
      if (currentPattern === "charger") return roll < 0.12 ? "charger" : roll < 0.34 ? "hover" : "skitter";
      return roll < 0.13 ? "hover" : "skitter";
    };

    const setPattern = (kind: ThreatKind) => {
      currentPattern = kind;
    ...
    const releaseSurge = () => {
      ...
      const kind = patterns[surgeIndex % patterns.length];
      const requested = Math.min(22, 8 + Math.floor(elapsed / 3));
      let spawned = 0;
      for (let index = 0; index < requested && foes.length < hordeHardCap; index += 1) {
        const surgeKind = kind === "skitter"
          ? (index % 7 === 6 ? "hover" : "skitter")
          : kind === "hover"
            ? (index % 3 === 0 ? "skitter" : "hover")
            : (index % 5 === 0 ? "charger" : index % 3 === 0 ? "hover" : "skitter");
        if (spawnFoe(surgeKind, index % 2 === 0 ? "left" : "right", index % 3)) spawned += 1;
      }
      surgesReleased += 1;
      surgeIndex += 1;
      surgeQueued = false;
      setPattern(kind);
      ring(320, arena.floor - 3, 210, true, 0.5);
      bannerMessage = `${patternLabel(kind)}  •  SURGE +${spawned}`;
      bannerClock = 1.25;
      playtest.event("horde_surge", {
        phase: "released",
        surge: surgesReleased,
        pattern: kind,
        added: spawned,
        active: foes.length,
        target: targetHordeAt(elapsed),
        second: Number(elapsed.toFixed(1)),
      });
    };
    ...
    const hurt = (source: "contact" | "rough_landing", contact?: Foe) => {
      ...
      playtest.event("death", {
        source,
        cleared: kills,
        powers_unlocked: [...unlocked],
        powers_triggered: [...triggered],
        stomp_spends: stompSpends,
        blocked_stomps: blockedStomps,
        horde_peak: hordePeak,
        surges: surgesReleased,
      });
      playtest.event("round_end", {
        outcome: "overrun",
        cleared: kills,
        powers: unlocked.size,
    ...
    const stomp = () => {
      ...
      if (stompEnergy < stompEnergyCost) {
        blockedStomps += 1;
        bannerMessage = "STOMP EMPTY  •  AUTO-RECOVERING";
        bannerClock = Math.max(bannerClock, 0.55);
        if (blockedStompTelemetryClock <= 0) {
          blockedStompTelemetryClock = 0.8;
          playtest.event("stomp_energy_blocked", {
            energy: Math.round(stompEnergy),
            cost: stompEnergyCost,
            blocked_total: blockedStomps,
            second: Number(elapsed.toFixed(1)),
          });
        }
        return;
      }
      const energyBefore = stompEnergy;
      stompEnergy = Math.max(0, stompEnergy - stompEnergyCost);
      stompEnergyDelayClock = stompEnergyDelay;
      stompSpends += 1;
      stompArmed = true;
      stompAge = 0;
      stompCooldown = 0.24;
      ...
      playtest.event("stomp_energy_spend", {
        energy_before: Math.round(energyBefore),
        energy_after: Math.round(stompEnergy),
        cost: stompEnergyCost,
        spend: stompSpends,
        second: Number(elapsed.toFixed(1)),
      });
    ...
    const update = k.onUpdate(() => {
      ...
      blockedStompTelemetryClock = Math.max(0, blockedStompTelemetryClock - dt);
      ...
      elapsed += dt;
      if (stompEnergyDelayClock > 0) {
        stompEnergyDelayClock = Math.max(0, stompEnergyDelayClock - dt);
      } else {
        stompEnergy = Math.min(stompEnergyMax, stompEnergy + stompEnergyRegen * dt);
      }

      const hordeTarget = targetHordeAt(elapsed);
      if (foes.length < hordeTarget) {
        refillBudget = Math.min(6, refillBudget + (8 + hordeTarget * 0.2) * dt);
        while (refillBudget >= 1 && foes.length < hordeTarget && foes.length < hordeHardCap) {
          const kind = refillKind();
          const laneIndex = kind === "hover" ? Math.floor(random() * 3) : 0;
          spawnFoe(kind, random() < 0.5 ? "left" : "right", laneIndex);
          refillBudget -= 1;
        }
      } else {
        refillBudget = Math.min(refillBudget, 1);
      }

      if (!surgeQueued && elapsed >= nextSurgeAt) {
        const patterns: ThreatKind[] = ["skitter", "hover", "charger"];
        const warningPattern = patterns[surgeIndex % patterns.length];
        surgeQueued = true;
        surgeTelegraphClock = 0.9;
        nextSurgeAt += 6.8;
        bannerMessage = `SURGE IN 1  •  ${patternLabel(warningPattern)}`;
        bannerClock = 0.9;
        playtest.event("horde_surge", {
          phase: "warning",
          surge: surgeIndex + 1,
      ...
      if (surgeQueued) {
        surgeTelegraphClock -= dt;
        if (surgeTelegraphClock <= 0) releaseSurge();
      }

      while (nextRampCheckpoint < rampCheckpoints.length && elapsed >= rampCheckpoints[nextRampCheckpoint]) {
        const checkpoint = rampCheckpoints[nextRampCheckpoint];
        playtest.event("horde_ramp", {
          checkpoint,
          active: foes.length,
          target: targetHordeAt(checkpoint),
          peak: hordePeak,
          second: Number(elapsed.toFixed(1)),
        });
        nextRampCheckpoint += 1;
      }
      hordePeak = Math.max(hordePeak, foes.length);
      ...
      const energyState = stompEnergy >= stompEnergyCost
        ? (stompEnergy >= stompEnergyMax - 0.5 ? "CHARGED" : "READY")
        : stompEnergy <= 0.5 ? "EMPTY" : stompEnergyDelayClock > 0 ? "SPENT" : "RECOVER";
      energyText.text = `STOMP ${Math.round(stompEnergy).toString().padStart(3, "0")}  ${energyState}`;
      energyText.color = stompEnergy >= stompEnergyCost ? accent : mid;
      energyFill.scale = k.vec2(Math.max(0.006, stompEnergy / stompEnergyMax), 1);
      energyFill.color = stompEnergy >= stompEnergyCost ? accent : mid;
      energyFill.opacity = stompEnergyDelayClock > 0 ? 0.5 : 0.8 + Math.sin(elapsed * 7) * 0.12;
      hordeText.text = `HORDE ${foes.length.toString().padStart(3, "0")}/${hordeTarget.toString().padStart(3, "0")}`;
      hordeText.color = foes.length > hordeTarget ? accent : pale;
